...
# Initialize working directory from environment variable
def initialize_working_directory():
    """Initialize working directory from MCP_WORKING_DIR environment variable"""
    working_dir = os.environ.get("MCP_WORKING_DIR")
    if working_dir and os.path.exists(working_dir):
        try:
            os.chdir(working_dir)
            print(f"MCP Terminal: Working directory set to {os.getcwd()}")
        except Exception as e:
            print(
                f"MCP Terminal: Failed to set working directory to {working_dir}: {e}"
            )
    else:
        print(f"MCP Terminal: Using default working directory {os.getcwd()}")
        ...
        if not isinstance(content, str):
            try:
                ...
                content = json.dumps(
                    ...
                    ensure_ascii=False,
                    default=lambda obj: (
                        str(obj) if hasattr(obj, "__dict__") else repr(obj)
                    ),
                )
            except Exception as e:
    ...
    try:
        ...
        if as_json:
            try:
                ...
                return json.dumps(
                    parsed_json, indent=4, sort_keys=False, ensure_ascii=False
                )
...
async def insert_file_content(
    path: str, content: str, row: int = None, rows: list = None
) -> str:
    """
    ...
    try:
        ...
        if not isinstance(content, str):
            try:
                ...
                content = json.dumps(
                    content, indent=4, sort_keys=False, ensure_ascii=False, default=str
                )
            except Exception as e:
...
async def delete_file_content(
    path: str, row: int = None, rows: list = None, substring: str = None
) -> str:
    """
    ...
    try:
        ...
        # Handle deleting a single row
            ...
            if row >= total_lines:
                return (
                    f"Error: Row {row} is out of range (file has {total_lines} lines)."
                )
...
async def update_file_content(
    path: str, content: str, row: int = None, rows: list = None, substring: str = None
) -> str:
    """
